...
def load_command_runs(connection: duckdb.DuckDBPyConnection, session_id: str) -> list[CommandRun]:
    ...
    return [
        CommandRun(
            ...
            tool_call_id=row[3],
            command=row[4],
            command_identity_hash=row[5],
            command_display=row[6],
            command_normalization=row[7],
            cwd=row[8],
            started_at=row[9],
            ended_at=row[10],
            exit_code=row[11],
            stdout_hash=row[12],
            stderr_hash=row[13],
            output_length=row[14],
            metadata=parse_metadata(row[15]),
        )
        for row in rows
    ]
...
def load_file_activities(
    ...
    return [
        FileActivity(
            ...
            project_relative_path=row[6],
            path_resolution=row[7],
            operation=row[8],
            timestamp=row[9],
            content_hash=row[10],
            metadata=parse_metadata(row[11]),
