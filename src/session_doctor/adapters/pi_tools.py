...
def tool_result_is_error(message_payload: dict[str, Any]) -> bool | None:
    ...
    if exit_code is not None:
        return exit_code != 0
    status = status_from_details(dict_value(message_payload.get("details")))
...
def status_from_details(value: object, *, depth: int = 0) -> str | None:
    ...
    for nested_value in payload.values():
        if isinstance(nested_value, dict):
            status = status_from_details(nested_value, depth=depth + 1)
            if status is not None:
                return status
    return None
